...
1. **学习/发射前缀写反**：氛围灯指令中，以下分支的 `xx` / `fs` 前缀与注释不符（学习用了 `fs`、发射用了 `xx`），会导致学习/发射行为异常：
   - `case 61/62/63`（学习氛围灯蓝/橙/青色）→ 发送了 `fs26/28/30`
   - `case 66/67`（学习氛围灯黄/白色）→ 发送了 `fs36/38`
   - `case 68/69`（氛围灯红/绿色）→ 发送了 `xx22/24`
   - `case 73/74`（氛围灯紫/粉色）→ 发送了 `xx32/34`
2. **回家模式 / 出门模式**（`case 177/178`）：尚未实现，`case 178` 仅有注释 TODO。
